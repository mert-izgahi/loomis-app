...
import { useGetProfileTrendingReports } from "@/hooks/use-profile";
import { formatDate } from "@/lib/utils";
import Link from "next/link";
import React from "react";

function TrendingReportsPage() {
  const { data, isLoading } = useGetProfileTrendingReports();
    ...
    <Container className="flex flex-col gap-8">
      <div className="flex flex-row items-start justify-betweenp-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold">Sık Kullanlan Raporlar</h1>
            ...
            return (
              ...
              >
                <div className="flex flex-row justify-between">
                  ...
                  <p className="text-xs">
                    {formatDate(item.report?.createdAt.toString())}
                  </p>
                </div>
                <div className="flex flex-col gap-1">
                  <p className="text-sm">{item.report?.description}</p>
                  <p className="text-sm">
                    Görüntülenme Sayısı: {item.viewCount}
                  </p>
                </div>
              </Link>
            );
